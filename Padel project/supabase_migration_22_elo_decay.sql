-- ============================================================
-- MIGRATION 22: Mécanisme de perte de points ELO pour inactivité (Elo Decay)
-- ============================================================

-- 1. Ajouter la colonne decay_applied_cycles à la table profiles s'il n'y a pas lieu
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS decay_applied_cycles INT DEFAULT 0;

-- 2. Recréer la fonction complete_match avec réinitialisation de decay_applied_cycles
...
BEGIN
  ...
  -- Mettre à jour ELO et réinitialiser les cycles de decay pour l'équipe 1
  ...
  LOOP
    ...
    SET elo_rating = GREATEST(0, elo_rating + ROUND(v_delta_t1)::INT),
        decay_applied_cycles = 0  -- Remise à zéro de l'inactivité compétitive
  ...
  -- Mettre à jour ELO et réinitialiser les cycles de decay pour l'équipe 2
  FOR v_participation IN
